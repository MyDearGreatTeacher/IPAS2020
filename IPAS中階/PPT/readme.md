# vuln hub  hacking articles
```

GreenOptic: 1 Vulnhub Walkthrough
https://www.vulnhub.com/entry/greenoptic_1,510/
```
# 技術
```
1.威脅分析與防禦技術
2.
3.其他
```
## 1.威脅分析與防禦技術
```
1.Webshell
2.攻擊藍芽
3.攻擊IOT
4.攻擊CLoud
5.攻擊mobile
6.攻擊web
7.微軟RDP漏洞
```
```
退信攻擊 手法分析及防禦技術
Cellopoint 發表新版「退信攻擊防禦」技術
http://www.cellopoint.com/tw/media_resources/news/cellopoint-pr-bounce-mail-prevent
```
```
微軟Windows遠端桌面服務存在安全漏洞(CVE-2019-0708)
https://www.ithome.com.tw/news/130646

[CVE-2019-0708] BlueKeep, RDP Remote Code Execution PoC
https://www.youtube.com/watch?v=zyC-IVKiS3E

BlueKeep - Exploit windows (RDP Vulnerability) Remote Code Execution
https://www.youtube.com/watch?v=y-KsMgswEuk
https://blog.rapid7.com/2019/09/06/initial-metasploit-exploit-module-for-bluekeep-cve-2019-0708/

https://pentest-tools.com/blog/bluekeep-exploit-metasploit/
```
```
勒索軟體攻擊及防禦

如何避免勒索軟體攻擊
Dell EMC Cyber Recovery 解決方案 企業對抗勒索軟體最佳選擇
遠離破壞性網路攻擊、避風港計畫再進化
文/廠商新聞稿 | 2020-04-06發表
https://www.ithome.com.tw/pr/136753
```
#### 1.Webshell
```
WebShell工具:中国蚁剑AntSword
http://kjol.cc/webshell-antsword.html

中国菜刀Webshell管理工具 归档
https://www.uedbox.com/post/51677/

向中国菜刀致敬！中国蚁剑：一款跨平台的开源网站管理工具
https://www.freebuf.com/sectool/98681.html
```
```
網絡間諜組織「海蓮花」打造全新MacOS後門程序
https://kknews.cc/tech/ovb2386.html
```
## 2.資安營運暨安全防護技術
```
資安營運 Windows 

使用 WindowsServer Update Services (WSUS) 來部署 Windows10 更新
https://docs.microsoft.com/zh-tw/windows/deployment/update/waas-manage-updates-wsus

Windows Server Update Services 最佳做法
https://support.microsoft.com/zh-tw/help/4490414/windows-server-update-services-best-practices

數位鑑識
```
### 滲透測試
```
M2
linux server 設透測試
https://community.rapid7.com/docs/DOC-1875

M3 
Windows作業系統與 server 設透測試
```
```
vuln. HUB

https://www.hackingarticles.in/

Comprehensive Guide on Broken Authentication & Session Management
https://www.hackingarticles.in/
```
```
在Kali Linux上安装PowerShell

解析APT29的無文件WMI和PowerShell後門
https://kknews.cc/tech/gvmbyky.html

```
```
渗透利器Cobalt Strike
https://cobaltstrike.com/
https://xz.aliyun.com/t/3975
https://kknews.cc/zh-tw/code/j2a54ly.html

「黑客工具」Cobalt Strike的滲透測試應用
https://kknews.cc/code/pve9y38.html

Cobalt strike3.0使用手冊
https://kknews.cc/code/kvb34jq.html

```
# 3.
```
OSCP Preparation Guide and Tips
https://www.youtube.com/watch?v=p4twsh9e4-A
```
